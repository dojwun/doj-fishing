...
QBCore.Functions.CreateUseableItem("fishtacklebox", function(source, item)
	...
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
		...
		SetTimeout(1250, function()
			Player.Functions.AddItem('pearlscard', 1, nil, info, {["quality"] = 100}) 
...
AddEventHandler("fishing:server:addTackleBox", function()
	...
	SetTimeout(1000, function()
		Player.Functions.AddItem('fishtacklebox', 1, nil, info, {["quality"] = 100}) 
    ...
    if itemFound then
        for i = 1, itemCount, 1 do
			...
			elseif luck >= 95 and luck <= 100 then
				Player.Functions.AddItem('fishinglootbig', 1, nil, info, {["quality"] = 100})
